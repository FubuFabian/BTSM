function [P, J] = RegionGrowingBreast(cIM, initPos, upThresVal, lowThresVal, maxDist, tfMean, tfFillHoles, tfSimplify)
...
% 
% % initial pixel value
% seedRegion = cIM(wx:wX,wy:wY,wz:wZ);
    ...
    for j = -R:R
      for k = -R:R
        ...
        if xv+i > 0  &&  xv+i <= nRow &&...          % within the x-bounds?
           ...
           cIM(xv+i, yv+j, zv+k) <= (regVal + upThresVal*regVal) &&...% within range %%%%%%%%%%%Modificar
           cIM(xv+i, yv+j, zv+k) >= (regVal - lowThresVal*regVal) &&...% of the threshold?
           isempty(find(J(nxs:nxe,nys:nye,nzs:nze)))
...
for cSli = 1:nSli
    ...
    if tfFillHoles
        ...
        B = bwboundaries(J(:,:,cSli), 4, 'noholes');
    else
	...
	newVertices = [B{max_index}(:,2), B{max_index}(:,1)];
