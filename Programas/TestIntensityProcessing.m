function [out] = TestIntensityProcessing(descriptor)
...
for i=1:nImages
    ...
    [imIntensity time(i)] = IntensityProcessing(imGray,descriptor);
        
    imwrite(imIntensity,strcat(savePath,'/Imagenes/Intensidad/',descriptor,'/',fileImages{i}(1:end-4),'_',descriptor,'.bmp'));    
    ...
    backgroundImage = imIntensity(background);
    ...
    for k=1:size(tumorEdge,1)
        ...
        epiTempFilter = epiTempFilter + abs(imIntensity(y,x)-imIntensity(y+1,x-1));
...
save(strcat(savePath,'/Probabilidades/Intensidad/',descriptor,'/','tumor_',descriptor,'.mat'),'meanTumorProbability');
save(strcat(savePath,'/Probabilidades/Intensidad/',descriptor,'/','background_',descriptor,'.mat'),'meanBackgroundProbability');
...
print (graf, '-dbmp', strcat(savePath,'/Graficas/Intensidad/',descriptor,'/','Probabilidad_',descriptor,'.bmp')) 